...
- **直感的なブックマーク管理**: 260種以上の豊富なアイコン選択（カテゴリ・検索対応）、登録後の編集、ドラッグ＆ドロップによる並び替えに対応。
...
- **豊富なアイコン選択**: 260種類以上の Material Symbols Rounded アイコンから自由に選択可能。8つのカテゴリ分類（Web、SNS、開発、仕事、メディア、生活、ツール等）とキーワードリアルタイム検索で素早くお好みのアイコンを指定。
...
- **初回利用規約同意フロー**: 初回アクセス時に利用規約・プライバシー方針の確認モーダルを表示し、同意ステータスおよび日時をスプレッドシートに永続化。
...
├── Code.gs            # サーバーサイド処理 (GAS API, スプレッドシート読み書き, ログ)
...
1. **コードの配置**:
   - エディタ左側のファイル一覧から `Code.gs` を開き、本リポジトリの `Code.gs` の内容を貼り付けて保存します。
...
- **v1.1.0 (2026/09/05)**:
  - **ブックマーク機能の全面刷新**:
    ...
    - 264種類の Material Symbols Rounded アイコンピッカー（8カテゴリ分類、日本語キーワード検索、件数表示対応）を搭載
  ...
  - **セキュリティ & データ管理の強化**:
    - ユーザー専用シートの自動保護機能（`protectUserSheet`：本人およびオーナー以外編集不可）を実装
    - 全ユーザーシートを一括保護するメンテナンス関数（`protectAllExistingUserSheets`）を追加
    - 初回利用規約・プライバシー方針の同意状態をスプレッドシート（`__TERMS__`）に自動永続化
- **v1.0.1 (2026/09/03)**:
